...
| 3 | **Evict compute** | Is any long-running command (builds, test suites, training, batch jobs) running inside a session, with the agent babysitting it? Move it to a driver (shell script, CI step, cron) between sessions: zero tokens, no timeouts, no died-while-waiting failures. (Read-path corollary: `readgate` denies redundant within-session re-reads of unchanged files — the read-path twin of the Lever 8 output filter.) | all compute-adjacent tokens |
...
| 7 | **Model arbitrage** | Reserve the big model for where judgment lives (strategy, synthesis, ambiguous calls). Route mechanical work — focused reviews, extraction, bulk edits, data crunching — to a small model. `token-diet route` makes this concrete: a deterministic rule table classifies a task into a tier, routing down only when confident and escalating high-stakes/ambiguous work to the top tier. Also: delegate a big-context agent's mechanical multi-turn work to ONE small-model agent. | 3–5× on the delegated share |
| 8 | **Filter tool output** | Are verbose command outputs (test suites, build logs, git status, listings, large file reads, **MCP server responses**) dumped whole into context, then re-sent every turn as cache reads? Filter at the source: a PostToolUse hook that compresses stdout per command kind (failures-only for tests, single-line confirms for git, head+tail+count for listings, structural crush for JSON/MCP, signatures-only for big reads). | often the largest cache-read source |
