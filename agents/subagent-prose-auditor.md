...
description: Opt-in specialist — tighten verbose wording (original to .verbose sidecar). Spawned only when a prose pass is explicitly requested.
model: sonnet
...
Opt-in: spawned only when a prose-tightening pass is explicitly requested. Read ONLY your rubric (`references/levers/lever-6-trim-always-loaded.md`)
